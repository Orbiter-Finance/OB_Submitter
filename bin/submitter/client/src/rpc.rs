    ...
    fn from(err: StateError) -> Self {
        JsonRpcError(match err {
            StateError::BincodeError(e) => {
                ErrorObject::owned(666, format!("error: {:#?}", e), None::<bool>)
            }
            ...
            _ => ErrorObject::owned(666, format!("error: unknown err"), None::<bool>),
        })
    }
}
...
impl SubmitterApiServer for SubmitterApiServerImpl<'static> {
    async fn get_balance(&self, address: Address) -> RpcResult<U256> {
        let state = self.state.read().map_err(|_| {
            ErrorObject::owned(888, format!("error: state read error."), None::<bool>)
        })?;
        let balance = state
