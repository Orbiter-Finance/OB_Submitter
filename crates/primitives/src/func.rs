use super::types::{BlockInfo, CrossTxDataAsKey};
...
pub fn tx_compare(a: &[u8], b: &[u8]) -> std::cmp::Ordering {
    let de_a = bincode::deserialize::<CrossTxDataAsKey>(a).unwrap();
    let de_b = bincode::deserialize::<CrossTxDataAsKey>(b).unwrap();
